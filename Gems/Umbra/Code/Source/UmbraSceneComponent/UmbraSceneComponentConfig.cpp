        ...
        if (auto serializeContext = azrtti_cast<AZ::SerializeContext*>(context))
        {
            ...
            if (auto editContext = serializeContext->GetEditContext())
            {
                editContext->Class<UmbraSceneComponentConfig>("UmbraSceneComponentConfig", "")
                    ...
                    ->DataElement(AZ::Edit::UIHandlers::Default, &UmbraSceneComponentConfig::m_sceneSettings, "Scene Settings", "Settings to configure how scene data is processed by the builder")
                    ->DataElement(AZ::Edit::UIHandlers::Default, &UmbraSceneComponentConfig::m_exportStaticObjectsOnly, "Export Static Objects Only", "If enabled, only objects with static transforms will be exported")
